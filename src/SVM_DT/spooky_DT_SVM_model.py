...
messages = preprocess(list(data['text'])[:83600])
...
with open('avgw2v_short_spooky.npy','wb') as f:
    np.save(f, vecs)

with open('labels_spooky.txt','w') as o:
    for i in labels:
        o.write(i+'\n')'''
...
with open('./Embeddings/avgw2v_short_spooky.npy','rb') as f:
    vecs = np.load(f)

with open('./Embeddings/labels/labels_spooky.txt','r') as o:
    while True:
...
'''with open('./Models/DT_model_spooky.sav','rb') as f:
  model = pickle.load(f)
pred = model.predict(X_test)
print(f1_score(y_test, pred, average='micro'))'''
...
'''with open('./Models/SVM_model_spooky.sav','rb') as f:
  model = pickle.load(f)
pred = model.predict(X_test)
print(f1_score(y_test, pred, average='micro'))'''
